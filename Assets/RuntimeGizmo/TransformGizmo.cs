...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		public static Action<Transform> OnAddTargetRoot;
		// public static Action<Transform> OnRemoveTargetRoot;
		public static Func<Transform, Transform> OnHitObject;
		public static Action OnClearObject;
		...
		void GetTarget()
		{
			if(nearAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
				...
				{
					Transform source = hitInfo.transform;
					Transform target = OnHitObject?.Invoke(source);
					if (target == null)
					{
						target = source;
					}
...
#if UNITY_EDITOR
			UnityEditor.Selection.activeObject = targetRoot.gameObject;
#endif
			AddAllChildren(targetRoot);
			OnAddTargetRoot?.Invoke(targetRoot);
		}
...
#if UNITY_EDITOR
				// UnityEditor.Selection.activeObject = targetRootsOrdered[];
#endif
				RemoveAllChildren(targetRoot);
			}
			// OnRemoveTargetRoot(targetRoot);
		}
